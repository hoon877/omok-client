...
public static class MCTSAIController
{
    ...
    // 보드 상태의 깊은 복사
    private static LConstants.PlayerType[,] CopyBoard(LConstants.PlayerType[,] board)
    {
        int rows = board.GetLength(0);
        int cols = board.GetLength(1);
        LConstants.PlayerType[,] newBoard = new LConstants.PlayerType[rows, cols];
        for (int i = 0; i < rows; i++)
        {
            for (int j = 0; j < cols; j++)
            {
    ...
    private static LConstants.PlayerType GetCurrentPlayer(LConstants.PlayerType[,] board)
    {
        int count = 0;
        int rows = board.GetLength(0);
        int cols = board.GetLength(1);
        ...
        return (count % 2 == 0) ? LConstants.PlayerType.PlayerA : LConstants.PlayerType.PlayerB;
    }

    // MCTS 트리의 노드를 표현하는 내부 클래스
    private class MCTSNode
    {
        public LConstants.PlayerType[,] board;
        ...
        public MCTSNode(LConstants.PlayerType[,] board, MCTSNode parent, (int row, int col) move)
        {
            this.board = board;
            this.parent = parent;
            this.move = move;
    ...
    // AI는 PlayerB로 가정합니다.
    private static LConstants.PlayerType DefaultPolicy(LConstants.PlayerType[,] board)
    {
        LConstants.PlayerType[,] simulationBoard = CopyBoard(board);
        LConstants.PlayerType currentPlayer = GetCurrentPlayer(simulationBoard);
        while (!IsTerminalState(simulationBoard))
        {
            ...
            currentPlayer = (currentPlayer == LConstants.PlayerType.PlayerA) ? LConstants.PlayerType.PlayerB : LConstants.PlayerType.PlayerA;
        }
        if (CheckGameWin(LConstants.PlayerType.PlayerB, simulationBoard))
            return LConstants.PlayerType.PlayerB;
        if (CheckGameWin(LConstants.PlayerType.PlayerA, simulationBoard))
            return LConstants.PlayerType.PlayerA;
        return LConstants.PlayerType.None;
    }

    // 역전파 단계: 시뮬레이션 결과를 루트까지 전파합니다.
    // 여기서는 AI가 PlayerB라고 가정하여, 시뮬레이션 결과가 PlayerB일 경우 승점을 추가합니다.
    private static void Backup(MCTSNode node, LConstants.PlayerType simulationResult)
    {
        while (node != null)
        {
            node.visits++;
            if (simulationResult == LConstants.PlayerType.PlayerB)
                node.wins++;
            node = node.parent;
        }
    }
    ...
    private static MCTSNode Expand(MCTSNode node)
    {
        ...
        LConstants.PlayerType[,] newBoard = CopyBoard(node.board);
        LConstants.PlayerType currentPlayer = GetCurrentPlayer(newBoard);
    ...
    public static (int row, int col)? GetBestMove(LConstants.PlayerType[,] board)
    {
        ...
        for (int i = 0; i < Iterations; i++)
        {
            ...
            LConstants.PlayerType result = DefaultPolicy(node.board);
            Backup(node, result);
        }
        MCTSNode bestChild = null;
        int bestVisits = -1;
    ...
    private static bool IsTerminalState(LConstants.PlayerType[,] board)
    {
        return CheckGameWin(LConstants.PlayerType.PlayerA, board) ||
               CheckGameWin(LConstants.PlayerType.PlayerB, board) ||
